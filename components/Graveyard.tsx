...
export const Graveyard: React.FC<GraveyardProps> = ({ playerGrave, npcGrave, isOpen: externalOpen, onToggle, restrictToMine }) => {
  ...
  const opponentLabel = t('common.opponent');
  const combinedWithTs = [
    ...playerGrave.map(c => ({ card: c, owner: youLabel, ts: (c as any).destroyedAt ?? 0 })),
    ...npcGrave.map(c => ({ card: c, owner: opponentLabel, ts: (c as any).destroyedAt ?? 0 }))
  ];
  ...
  const entriesToShow = (() => {
    if (restrictToMine) return combined.filter(e => e.owner === youLabel);
    if (filter === 'mine') return combined.filter(e => e.owner === youLabel);
    if (filter === 'opponent') return combined.filter(e => e.owner === opponentLabel);
    return combined;
  })();
  ...
  return (
    <div className={`fixed right-0 top-[240px] sm:top-[290px] z-[33] transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-[calc(100%-56px)]'} ${isOpen ? '' : 'pointer-events-none'}`}>
      <div className="flex items-start">
        <button onClick={toggle} className="bg-slate-800 border-2 border-r-0 border-white/20 p-4 rounded-l-2xl shadow-2xl hover:bg-slate-700 transition-colors pointer-events-auto" aria-label={t('graveyard.openGraveyard')}>
        ...
        <div className="w-80 sm:w-[40vw] h-[60vh] sm:h-auto bg-slate-900/95 border-2 border-white/20 shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-md flex flex-col rounded-l-none rounded-r-lg p-2 overflow-hidden">
          <div className="flex items-center justify-between px-4 border-b border-white/5">
            <div className="font-black text-lg text-slate-100">{t('game.graveyard')}</div>
            <div className="text-xs text-slate-400">{t('graveyard.cardCount', { count: entriesToShow.length })}</div>
          </div>

          <div className="p-3 border-b border-white/5 flex gap-2 items-center">
            {!restrictToMine && (
              <>
                <button onClick={() => setFilter('all')} className={`px-2 py-1 rounded ${filter === 'all' ? 'text-yellow-600 font-bold' : 'bg-slate-800 text-slate-200'}`}>{t('graveyard.filterAll')}</button>
                <button onClick={() => setFilter('mine')} className={`px-2 py-1 rounded ${filter === 'mine' ? 'text-blue-600 font-bold' : 'bg-slate-800 text-slate-200'}`}>{t('graveyard.filterMine')}</button>
                <button onClick={() => setFilter('opponent')} className={`px-2 py-1 rounded ${filter === 'opponent' ? 'text-red-600 font-bold' : 'bg-slate-800 text-slate-200'}`}>{t('graveyard.filterOpponent')}</button>
              </>
            )}
            {restrictToMine && (
              <div className="text-sm text-slate-300 font-semibold">{t('graveyard.restrictedView')}</div>
            )}
          </div>

          <div className="flex-1 p-3 flex overflow-x-auto">
            {entriesToShow.length === 0 && (
              <div className="h-full flex items-center justify-center text-slate-600 font-black italic text-center p-8">{t('graveyard.empty')}</div>
            )}

              {entriesToShow.slice().reverse().map((entry, idx) => (
                <div key={`${entry.card.uniqueId}-${idx}`} className={`p-2 rounded-xl bg-slate-800/40 border-l-4 scale-95 h-30 ${entry.owner !== opponentLabel ? 'border-blue-500' : 'border-red-500'}`}>
                  <CardComponent
