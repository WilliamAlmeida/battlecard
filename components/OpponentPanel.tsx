  ...
  return (
    <div className={`fixed right-0 top-[305px] sm:top-[365px] z-[32] transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-[calc(100%-56px)]'} ${isOpen ? '' : 'pointer-events-none'}`}>
      <div className="flex items-start">
        <button onClick={toggle} className="bg-slate-800 border-2 border-r-0 border-white/20 p-4 rounded-l-2xl shadow-2xl hover:bg-slate-700 transition-colors pointer-events-auto" aria-label={t('opponentPanel.openPanel')}>
        ...
        <div className="w-80 sm:w-[40vw] h-[60vh] sm:h-auto bg-slate-900/95 border-2 border-white/20 shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-md flex flex-col rounded-l-none rounded-r-lg p-2 overflow-hidden">
          <div className="flex items-center justify-between px-4 border-b border-white/5">
            <div className="font-black text-lg text-slate-100">{t('common.opponent')}</div>
            <div className="text-xs text-slate-400">{t('game.hand')}: {npcHand.length} • {t('game.deck')}: {npcDeck.length}</div>
          </div>

          <div className="p-3 border-b border-white/5 flex gap-2 items-center">
            <button onClick={() => setReveal(false)} className={`px-2 py-1 rounded ${!reveal ? 'text-yellow-600 font-bold' : 'bg-slate-800 text-slate-200'}`}>{t('opponentPanel.hide')}</button>
            <button onClick={() => setReveal(true)} className={`px-2 py-1 rounded ${reveal ? 'text-blue-600 font-bold' : 'bg-slate-800 text-slate-200'}`}>{t('opponentPanel.reveal')}</button>
            <div className="ml-auto text-sm text-slate-400">({t('opponentPanel.devModeHint')})</div>
          </div>

          <div className="flex-1 p-3 flex flex-col gap-3 overflow-auto">
            <div>
              <div className="font-semibold text-slate-300 mb-2">{t('opponentPanel.opponentHand')}</div>
              <div className="flex gap-2 overflow-x-auto pb-2">
                {npcHand.length === 0 && <div className="text-slate-600 italic">{t('opponentPanel.emptyHand')}</div>}
                {npcHand.map((c, i) => (
            ...
            <div>
              <div className="font-semibold text-slate-300 mb-2">{t('opponentPanel.opponentDeck', { count: npcDeck.length })}</div>
              <div className="flex gap-2 items-center">
                <div className="flex flex-col items-center">
                  ...
                  <div className="text-xs text-slate-400 mt-1">{t('opponentPanel.cardsCount', { count: npcDeck.length })}</div>
                </div>
                ...
                <div className="flex-1">
                  {reveal ? (
                    ...
                    <div className="text-slate-500 italic">{t('opponentPanel.deckHidden')}</div>
                  )}
                </div>
              </div>
            </div>
