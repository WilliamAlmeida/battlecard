              ...
              <thead className="sticky top-0 bg-slate-900/95 z-10">
                <tr>
                  ...
                  {Object.values(ElementType).map((def) => (
                    <th key={def} className="p-2 text-center font-semibold text-slate-200">{def}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {Object.values(ElementType).map((atk) => (
                  <tr key={atk} className="border-t border-white/5 hover:bg-slate-800/60 transition-colors">
                    <td className="p-2 font-bold text-slate-100 sticky left-0 bg-slate-900/95">{atk}</td>
                    {Object.values(ElementType).map((def) => {
